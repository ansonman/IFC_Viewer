...
## 12. 2025-10-06：PSC P2 互動與診斷開關（現況）

本次聚焦於簡化互動與提升穩定性：
- 移除橡皮筋框選與多選；保留單擊選取。
- 滑鼠位置為中心的滾輪縮放（全畫布任意位置適用）與中鍵平移。
- 選取同步 3D 高亮：每次選取會先清除前次高亮，再套用新的選取集合。
- 新增「顯示縮放錨點」診斷開關（預設關閉），勾選後，每次縮放會短暫顯示藍色錨點以協助驗證縮放錨點。

已知狀態：
- 建置成功（僅 NU1701 相容性警告）。
- 現場測試「未能完全成功」，已於 DEVELOPMENT_LOG 紀錄可能根因與後續 Debug 計畫（含 [FTA] 日誌點位）。

待辦（短期）：
1) 針對縮放錨點與 3D 高亮同步補強日誌，重現並定位問題。
2) 若藍點標記導致互動不穩，保留開關但預設關閉、僅於診斷時啟用。
3) P2 視圖加上快速搜尋定位（關鍵字→篩選/置中）。
