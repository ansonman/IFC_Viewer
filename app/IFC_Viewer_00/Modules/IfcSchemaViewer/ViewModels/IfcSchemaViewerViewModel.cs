...
namespace IfcSchemaViewer.ViewModels
{
    public class IfcSchemaNode : ObservableObject
    {
        ...
        // Lazy materialization support for large nodes
        internal Func<IEnumerable<IfcSchemaNode>>? LazyFactory { get; set; }
        public bool HasLazy => LazyFactory != null;
        public void EnsureMaterialized()
        {
            if (LazyFactory != null)
            {
                try
                {
                    var items = LazyFactory.Invoke() ?? Enumerable.Empty<IfcSchemaNode>();
                    Children.Clear();
                    foreach (var c in items) Children.Add(c);
                }
                catch { }
                finally { LazyFactory = null; }
            }
        }
    }
    ...
    // Persist/restore expansion state across entity switches
    private readonly HashSet<string> _expandedPaths = new(StringComparer.OrdinalIgnoreCase);
    private bool _hasShownContent = false; // for first-open smart expand

        public void LoadFrom(IIfcObject entity)
        {
            // snapshot current expansion from ViewNodes
            CaptureExpandedPathsFromView();
            ...
            // 延遲渲染大型節點：以 LazyFactory 延後子節點建立，直到展開時再生成
            var attrsNode = new IfcSchemaNode { Name = "Attributes" };
            attrsNode.LazyFactory = () => BuildAttributesChildren(entity);
            root.Children.Add(attrsNode);

            var invNode = new IfcSchemaNode { Name = "Inverses" };
            invNode.LazyFactory = () => BuildInversesChildren(entity);
            root.Children.Add(invNode);

            var psetsNode = new IfcSchemaNode { Name = "PropertySets" };
            psetsNode.LazyFactory = () => BuildPropertySetsChildren(entity);
            root.Children.Add(psetsNode);

            RootNodes.Add(root);
            ApplyFilter();
            Title = $"IFC Schema Viewer - {TryGetTypeName(entity)}";

            // 還原展開狀態；若首次開啟且無快照，做智慧展開（根與第一層）
            if (_expandedPaths.Count > 0)
            {
                ApplyExpandedPathsTo(ViewNodes);
            }
            else if (!_hasShownContent)
            {
                SmartExpand(ViewNodes, maxDepth: 1);
            }
        ...
        public void ApplyFilter()
        {
            ...
            if (string.IsNullOrEmpty(text))
            {
                ...
                {
                    // 複製顯示樹，保留 IsExpanded；必要時延遲節點於展開時生成
                    ViewNodes.Add(CloneShallow(r, includeChildren: true));
                }
                return;
            }
            ...
            {
                // 為了搜尋，若節點是延遲載入，需先實體化子項再過濾
                n.EnsureMaterialized();
                var keep = Match(n);
                var copy = new IfcSchemaNode { Name = n.Name, Value = n.Value, IsExpanded = n.IsExpanded };
        ...
        {
            var copy = new IfcSchemaNode { Name = n.Name, Value = n.Value, IsExpanded = n.IsExpanded };
            if (n.HasLazy) copy.LazyFactory = n.LazyFactory; // 保留延遲載入行為
            if (includeChildren)
            {
                // 若節點已標示展開，先實體化其子項再複製；否則保留延遲
                if (copy.IsExpanded) n.EnsureMaterialized();
                foreach (var c in n.Children)
        ...
        private void CaptureExpandedPathsFromView()
        {
            try
            {
                _expandedPaths.Clear();
                void Walk(IList<IfcSchemaNode> nodes, string parent, bool materialize)
                {
                    for (int i = 0; i < nodes.Count; i++)
                    {
                        ...
                        var path = string.IsNullOrEmpty(parent)
                            ? $"{San(n.NameOnly)}[{i}]"
                            : $"{parent}/{San(n.NameOnly)}[{i}]";
                        if (n.IsExpanded) _expandedPaths.Add(path);
                        if (materialize && n.IsExpanded) n.EnsureMaterialized();
                        if (n.Children != null && n.Children.Count > 0)
                            Walk(n.Children, path, materialize);
                    }
                }
                Walk(ViewNodes, string.Empty, materialize: false);
            }
            catch { }
        }

        private void ApplyExpandedPathsTo(IList<IfcSchemaNode> nodes)
        {
            try
            {
                void Walk(IList<IfcSchemaNode> ns, string parent)
                {
        ...
        private static string San(string s) => s?.Replace('/', '\\') ?? string.Empty;

        private void SmartExpand(IList<IfcSchemaNode> nodes, int maxDepth)
        {
            try
            {
                void Walk(IList<IfcSchemaNode> ns, int depth)
                {
                    for (int i = 0; i < ns.Count; i++)
                    {
                        var n = ns[i];
        ...
        private static IEnumerable<IfcSchemaNode> BuildAttributesChildren(IIfcObject entity)
        {
            var list = new List<IfcSchemaNode>();
            try
            {
                        ...
                        foreach (var p in props)
                        {
                            try
                            {
                                ...
                                if (!string.IsNullOrWhiteSpace(pName) && pi != null)
                                {
                                    ...
                                    list.Add(new IfcSchemaNode { Name = pName!, Value = val });
                                }
                            }
                            catch { }
                        }
            ...
            try
            {
                foreach (var pi in entity.GetType().GetProperties(BindingFlags.Public | BindingFlags.Instance))
                {
                    ...
                    try
                    {
                        ...
                        list.Add(new IfcSchemaNode { Name = pi.Name, Value = str });
                    }
                    catch { }
                }
            }
        ...
        private static IEnumerable<IfcSchemaNode> BuildInversesChildren(IIfcObject entity)
        {
            var list = new List<IfcSchemaNode>();
            try
            {
                        ...
                        foreach (var inv in invs)
                        {
                            try
                            {
                                ...
                                if (!string.IsNullOrWhiteSpace(name) && pi != null)
                                {
                                    ...
                                    list.Add(new IfcSchemaNode { Name = name!, Value = str });
                                }
                            }
                            catch { }
                        }
        ...
        private static IEnumerable<IfcSchemaNode> BuildPropertySetsChildren(IIfcObject entity)
        {
            var list = new List<IfcSchemaNode>();
            try
            {
